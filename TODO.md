...
| 2026-09-24 | Phase 4.7 started: environment audit found Vina f458505-mod and Meeko 0.7.1 in cadd; ProDy is absent. Implemented strict Vina sampling parameters, argv-only box command planning, parser for pose score remarks, and ligand-efficiency calculation. Adapter tests: 9 passed; full gate: 238 passed, 4 engine-specific tests skipped; Ruff, strict mypy (82 source files), import-linter, and schemas pass. Meeko preparation, worker execution, and pose normalization remain. The PDB derivative is now registered separately while mmCIF remains authoritative; PDBFixer→Meeko receptor prep was run successfully on the 5NIU fixture. Engine-enabled handler test covers the same handoff and confirms Meeko writes PDBQT plus parameter JSON. Validation: full gate 238 passed/4 engine skips; engine-enabled preparation suite 7 passed; Ruff, format, strict mypy (82 files), import-linter, and schemas pass. |
