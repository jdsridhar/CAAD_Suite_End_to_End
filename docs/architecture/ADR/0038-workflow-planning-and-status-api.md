...
- **Validation:** API tests cover auth, plugin capability output, successful Vina plan compilation, successful normalized workflow execution with an injected test adapter, duplicate run rejection, project-scoped run status, SSE completion, and provenance queries.
